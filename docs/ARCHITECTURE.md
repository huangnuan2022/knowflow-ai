...
- Expanded nodes may scroll their message thread, so scroll events must refresh React Flow node internals for the selected node. Collapsed branch-point lists may also scroll. Branch edges should render only when their source highlight or branch-point chip is currently visible; hidden-source branch edges should be temporarily omitted so edges do not appear to originate from empty scroll space.
...
- Manual edges are allowed for non-branch peer relationships between collapsed node-level side handles and may have editable labels and delete controls. Collapsed nodes expose manual handles on all four sides, and a manual edge gesture may start from any side and finish on any other collapsed node side. The frontend normalizes that gesture into an undirected node-level relationship; the backend should not infer semantic direction from which side the user dragged from. Persisted manual edges are rendered from the closest source/target sides based on node positions. They should remain visually and semantically separate from branch edges. Branch points should not be manual connection handles. Branch edge labels represent selected source text and should not be freely edited in v0.
