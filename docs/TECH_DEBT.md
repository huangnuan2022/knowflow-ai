...
| Stub AI provider is non-production | Phase 1 needs a provider-neutral run lifecycle without connecting to a real model. | Stub responses can be mistaken for product AI behavior if exposed in UI. | Implementing the selected OpenAI Responses API adapter. | Keep `stub` for tests, but make `openai` the real runtime provider behind the same interface. |
