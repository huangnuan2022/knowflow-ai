...
| Simple context summaries | Full graph context would explode token usage. | Summaries may omit important details. | Users see irrelevant or incomplete AI answers. | Add user-controlled context selection and richer context snapshots. |
...
| Branch command creates placeholder Run provider/model | The branch workflow needs a ContextSnapshot boundary before AI provider integration exists. | Run records can be mistaken for real AI provider requests. | The provider-neutral AI adapter is implemented. | Replace `not-selected` provider/model placeholders with real adapter-owned run creation metadata. |
