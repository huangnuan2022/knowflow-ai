...
| Stub AI provider is non-production | Phase 1 needs a provider-neutral run lifecycle without connecting to a real model. | Stub responses can be mistaken for product AI behavior if exposed in UI. | The first real provider and model are selected. | Replace `stub` with a real provider adapter behind the same interface. |
