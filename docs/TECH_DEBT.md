...
| Conversation panel displays plain selectable text | This keeps the first node-thread UI small while preserving selectable content for the upcoming branch workflow. | Markdown-specific formatting, code blocks, and selection offsets may need refinement before polished learning demos. | Users ask questions with code snippets, or branch selection is implemented in the UI. | Add a Markdown renderer with stable text-selection anchoring and code-block styling. |
